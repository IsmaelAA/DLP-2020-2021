...
public class CodeSelection extends DefaultVisitor {
	...
	public Object visit(DefFuncion node, Object param) {
		...
		int localesAux = 0;
		// Inicializar variables si se tiene que hacer.
		for (DefVariable child : node.getDefvariable()) {
			
			if (child.getExpression() != null) {
				localesAux += child.getType().getMemSize();
				out("pusha BP");
				out("push " + -(localesAux));
				out("add");
				child.getExpression().accept(this, Funcion.VALOR);
				out("store" + child.getExpression().getType().getSufijo());
			}
		}
	...
	public Object visit(VerStruct node, Object param) {
		...
		if (Funcion.VALOR.equals(param)) {
			...
			node.getName().accept(this, Funcion.DIRECCION);
			List<VarStruct> lista = ((StructType) (node.getName()).getType()).getDefinicion().getVarstruct();
