...
public class ASTPrinter extends DefaultVisitor {
	...
	//	class For { Sentence sentencia;  Expression condition;  Sentence step;  List<Sentence> sentence; }
	public Object visit(For node, Object param) {
		int indent = ((Integer)param).intValue();

		printName(indent, "For", node, false);

		visit(indent + 1, "sentencia", "Sentence",node.getSentencia());
		visit(indent + 1, "condition", "Expression",node.getCondition());
		visit(indent + 1, "step", "Sentence",node.getStep());
		visit(indent + 1, "sentence", "List<Sentence>",node.getSentence());
		return null;
	}
