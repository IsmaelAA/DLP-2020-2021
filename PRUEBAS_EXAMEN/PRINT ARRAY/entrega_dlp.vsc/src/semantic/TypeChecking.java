...
public class TypeChecking extends DefaultVisitor {
	...
	if (node.getPrint().getType() instanceof ArrayType) {
	    ...
	    predicado(((ArrayType) node.getPrint().getType()).getType() instanceof CharType,
		    "Error, el print debe de ser de un array de char.", node);
	...
	if (node.getLeft().getType() instanceof ArrayType) {
	    ...
	    predicado(node.getRight().getType() instanceof ArrayType,
		    "Error, solo se puede asignar un array a otro array.", node);
	    predicado(((ArrayType) node.getLeft().getType()).getType() instanceof CharType,
		    "Error, el tipo de ambos arrays debe de ser char.", node);
	    predicado(
		    Integer.parseInt(((ArrayType) node.getLeft().getType()).getPosition()) <= Integer
			    .parseInt(((ArrayType) node.getRight().getType()).getPosition()),
		    "Error, el tamaño del array de la derecha es inferior al de la izquierda, imposible asignar.");
	} else
	...
	predicado(node.getName().getType() instanceof IdentType, "Error, tipo de struct necesario", node);
	node.setModificable(true);

	if (node.getName().getType() instanceof IdentType) {
	    IdentType st = (IdentType) node.getName().getType();
